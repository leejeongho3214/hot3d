...
seq_name = "dataset/P0001_4bf4e21a"
...
name_json = home + f"/Desktop/analysis/{seq_name.split('/')[-1]}.json"
...
for object_name, value_list in tqdm(content['single'].items()):
    vertex_color_total = defaultdict()
    if object_name != "whiteboard_marker":
        continue
    
    for a_id, value in tqdm(enumerate(value_list)):
        
        # object_name = "whiteboard_eraser"
        # value[1][0], value[1][1] = 1000, 1054
        for idx, frame_idx in enumerate(tqdm(range(value[1][0], value[1][1]))):
            ...
            for obj_id, _ in object_poses_with_dt.pose3d_collection.poses.items():
                if object_library.object_id_to_name_dict[obj_id] != object_name:
                    continue
                ...
                base_label = f"/{object_name}/{value[1][0]}~{value[1][1]}"
                rr.log(
